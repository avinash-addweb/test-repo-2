        ...
        <div class="col-md-12">
          	<div class="card">
            	<div class="card-body">
					<div class="row">
						<div class="col-md-10">
							<h5 class="card-title">{{__('Edit :record',['record'=>__('User')])}}</h5>
						</div>
					...
					<div class="row">
						<div class="col-md-12">
							...
							<form method="POST" action="{{ route('user.update', $user->id) }}">
								...
								@method('PUT')
                                <div class="row mb-3">
                                    <label for="name" class="col-sm-3 col-form-label">{{ __('Employee ID') }} :</label>
                                    <div class="col-sm-9">
                                        <input type="input" class="form-control" name="adminId" id="adminId"  value="{{ old('adminId', $user->adminId) }}" readonly disabled>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-3">
									...
									<div class="col-sm-9">
										<input type="input" class="form-control @error('first_name') is-invalid @enderror" name="first_name" id="first_name"  value="{{ old('first_name', $user->first_name) }}">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="input" class="form-control @error('middle_name') is-invalid @enderror" name="middle_name" id="middle_name"  value="{{ old('middle_name', $user->middle_name) }}">
                                    </div>
                                </div>
                                <div class="row mb-1">&nbsp;</div>
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="input" class="form-control @error('last_name') is-invalid @enderror" name="last_name" id="last_name"  value="{{ old('last_name', $user->last_name) }}">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="date" class="form-control mydatepicker @error('date_of_birth') is-invalid @enderror" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $user->date_of_birth) }}" max="{{date('d/m/Y')}}">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="input" class="form-control @error('mobile_number') is-invalid @enderror" name="mobile_number" id="mobile_number"  value="{{ old('mobile_number', $user->mobile_number) }}">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="input" class="form-control @error('whatsapp_number') is-invalid @enderror" name="whatsapp_number" id="whatsapp_number"  value="{{ old('whatsapp_number', $user->whatsapp_number) }}">
                                        @error('whatsapp_number')<span class="invalid-feedback" role="alert">{{ $message }}</span>@enderror
                                    </div>
                                </div>
                                <div class="row mb-1">&nbsp;</div>
								...
								<div class="row mb-3">
									...
									<div class="col-sm-9">
										<input type="input" class="form-control @error('email') is-invalid @enderror" name="email" id="email"  value="{{ old('email', $user->email) }}">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" name="designation_id" id="designation_id">
                                            ...
                                            @foreach($designations as $designation)
                                                <option value="{{ $designation->id }}" {{ $designation->id == old('designation_id', $user->designation_id) ? "selected" : null }}>{{ $designation->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" name="department_id" id="department_id">
                                            ...
                                            @foreach($departments as $department)
                                                <option value="{{ $department->id }}" {{ $department->id == old('department_id', $user->department_id) ? "selected" : null }}>{{ $department->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" name="loan_type_id[]" id="loan_type_id" multiple>
                                            @foreach($loan_types as $loan_type)
                                                <option value="{{ $loan_type->id }}" {{ !empty($userHasLoanTypes) && in_array($loan_type->id, $userHasLoanTypes)  ? "selected" : null }}>{{ $loan_type->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-select" name="region_id" id="region_id">
                                            ...
                                            @foreach($regions as $region)
                                                <option value="{{ $region->id }}" {{ $region->id == old('region_id', $user->region_id) ? "selected" : null }}>{{ $region->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
									...
									<div class="col-sm-9">
										@forelse ($roles as $role)
											...
											<span class="form-check1 ml-3">
												<input type="checkbox" name="roles[]" id="{{ $role->name }}" value="{{ $role->name }}" class="form-check-input" {{ in_array($role->id, $userHasRoles) ? 'checked' : '' }}>
								...
								<fieldset class="row mb-3">
									...
									<div class="col-sm-9">
										...
										<span class="form-check1">
											<input class="form-check-input" type="radio" name="status" id="status2" value="2" @if($user->status != 1) checked @endif>
